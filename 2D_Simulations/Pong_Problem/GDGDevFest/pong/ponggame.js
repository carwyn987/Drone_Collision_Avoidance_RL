...
//from pong code:
var update = function () {
    player.update(); // Moves player paddle based on user interface
    console.log(computer.ai_plays)
    if(computer.ai_plays){
        move = ai.predict_move(); // Choose a move for paddle based off last_data_object
        computer.ai_update(move); // Just moves paddle
    }else
        computer.update(ball); // deterministic move paddle
    ball.update(player.paddle, computer.paddle); // update ball position
