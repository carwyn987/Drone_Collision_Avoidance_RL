...
def computeReward(pos_vector, collision):
    # print(pos_vector.z_val)
    if abs(pos_vector.y_val) < 1 and not collision and abs(pos_vector.z_val) < 2:
        return (1, collision)
    elif not collision and abs(pos_vector.z_val) < 5:
        return (0, collision)
    else:
        return (-1, True)
...
# Save model
torch.save(behavior_model.state_dict(), "dqn_hover_behavior_general.pth")
torch.save(target_model.state_dict(), "dqn_hover_target_general.pth")
