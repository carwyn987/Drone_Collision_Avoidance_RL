...
def computeReward(pos_vector, collision):
    if not collision and abs(pos_vector.z_val) < 4 and abs(pos_vector.y_val) < 1 and abs(pos_vector.z_val) > 3:
        return 2, collision
    elif not collision and abs(pos_vector.y_val) < 2:
        return 1, False
    else:
        return 0, True
...
# Save model
torch.save(behavior_model.state_dict(), "../models/dqn_dodge_behavior_point_yz.pth")
torch.save(behavior_model_roll.state_dict(), "../models/dqn_dodge_behavior_roll_point_yz.pth")