...
def computeReward(pos_vector, collision):
    if not collision:
        r = 0
        if abs(pos_vector.z_val) < 0.5 or abs(pos_vector.z_val) > 6 or abs(pos_vector.y_val) > 4:
            return 0, False
        elif 3 - abs(pos_vector.z_val) > 0.5:
            r += 1 / (3 - abs(pos_vector.z_val))
        else:
            r += 3
        if abs(pos_vector.y_val) > 0.5:
            r += 1 / (abs(pos_vector.y_val))
        else:
            r += 3
        return r, False
    else:
...
# Save model
torch.save(behavior_model.state_dict(), "models/dqn_dodge_full_gradient_throttle.pth")
torch.save(behavior_model_roll.state_dict(), "models/dqn_dodge_full_gradient_roll.pth")