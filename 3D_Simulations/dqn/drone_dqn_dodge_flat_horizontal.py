...
def computeReward(pos_vector, collision):
    if not collision and abs(pos_vector.z_val) < 3.5 and abs(pos_vector.y_val) < 3 and abs(pos_vector.z_val) > 3:
        return 2, False
    elif abs(pos_vector.y_val) > 7 or abs(pos_vector.z_val) > 10:
        return 0, True
    elif not collision:
        return 0.1, False
    else:
...
# Save model
torch.save(behavior_model.state_dict(), "../models/dqn_dodge_behavior_flat_horizontal_yz.pth")
torch.save(behavior_model_roll.state_dict(), "../models/dqn_dodge_behavior_roll_flat_horizontal_yz.pth")