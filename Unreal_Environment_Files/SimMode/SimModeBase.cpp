...
void ASimModeBase::spawnBulletOnTimer(){
    shootCounter += 1;
    if(shootCounter == 200){ // Approximately every 6 seconds
        shootCounter = 0;
        ASimModeBase::Shoot();
    }
}
...
{
    UE_LOG(LogTemp, Warning, TEXT("RESET RESET"));
...
void ASimModeBase::Shoot()
{
	UE_LOG(LogTemp, Warning, TEXT("Shoot bullet projectile"));

    // Compute transform to spawn bullet near drone actor
    ...
    GetActorEyesViewPoint(CameraLocation, CameraRotation);
        ...
        // move ball to the +X axis in front of player
        player_loc.X += 1500.0f;
    ...
    GetWorld()->SpawnActor<ABullet>(ABullet::StaticClass(), player_loc, MuzzleRotation, SpawnParams);
...
void ASimModeBase::setupInputBindings()
{
    ...
    UAirBlueprintLib::BindActionToKey("Shoot", EKeys::E, this, &ASimModeBase::Shoot);
}
